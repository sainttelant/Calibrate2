...
namespace UcitCalibrate
{
	...
	void CalibrationTool::Gps2WorldCoord(std::vector<double> P1_lo, std::vector<double> P1_la)
	{
		...
		for (int i = 0; i < P1_la.size(); i++)
		{
			...
			GpsWorldtmp.X = 2 * m_PI * (m_earthR_Polar * cos(m_originlatitude * val)) * ((P1_lo[i] - m_originlongitude) / 360);
			GpsWorldtmp.Y = 2 * m_PI * m_earthR * ((P1_la[i] - m_originlatitude) / 360);
			GpsWorldtmp.Altitude = sqrt(GpsWorldtmp.X * GpsWorldtmp.X + GpsWorldtmp.Y * GpsWorldtmp.Y);
			printf("P[%d]::Gps world:x=%.10f Gps World y=%.10f Distance of world=%.10f \n", i + 1, GpsWorldtmp.X, GpsWorldtmp.Y, \
			GpsWorldtmp.Altitude);
			m_gpsworlds.push_back(GpsWorldtmp);
		}
	...
	void CalibrationTool::Gps2radarworld(longandlat& m_gpslongandlat, GpsWorldCoord& m_gpsworldcoord)
	{
		double val = m_PI / 180.0;
			...
			temp.Altitude = m_gpslongandlat.altitude;
			...
			m_gpsworldcoord.Altitude = radar_Dis.at<double>(2, 0);
	...
	void CalibrationTool::Gps2Pixel(longandlat& m_gps, cv::Point2d& m_pixels)
	{
		double val = m_PI / 180.0;
		GpsWorldCoord temp;
		temp.X = 2 * m_PI * (m_earthR_Polar * cos(m_originlatitude * val)) * ((m_gps.longtitude - m_originlongitude) / 360);
		temp.Y = 2 * m_PI * m_earthR * ((m_gps.latitude - m_originlatitude) / 360);
		temp.Altitude = m_gps.altitude;

		Mat world_point = Mat::ones(4, 1, cv::DataType<double>::type);
		world_point.at<double>(0, 0) = temp.X;
		world_point.at<double>(1, 0) = temp.Y;
		world_point.at<double>(2, 0) = temp.Altitude;
